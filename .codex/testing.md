...
### 2025-11-26（Codex）backend-python 系统日志中间件修复
- 调整 `backend-python/app/logging_middleware.py`：
  - 为 `/monitor/online` 与 `/system/log` 增加模块映射，并在未匹配到路由前缀时统一归类为“其它”。
  - 写入 sys_log 时强制使用非空模块字符串，避免 `module` 列 NOT NULL 约束导致插入失败。
- 验证：在本环境完成 `python3 -m py_compile $(git ls-files "backend-python/**/*.py")` 语法检查通过；需在本地启动 FastAPI 服务后，通过登录、用户增删改等接口实际访问，确认 `sys_log` 中新增记录模块与描述正常。
